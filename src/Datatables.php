...
class Datatables {
	...
	{
		if (!empty($this->options['scope'])) $this->applyScope();

		if (!empty($this->options['extraWhere'])) $this->applyExtraWhere();

		if (method_exists($this->model, 'scopeEagerLoading')) $this->queryBuilder->eagerLoading();

		if (isset($this->options['order']) && (!empty($this->options['order'][0]['column']) || $this->options['order'][0]['column'] === '0')) $this->sortByColumn();
	...
	private function applyScope()
	{
		...
		{
			if (method_exists($this->model, 'scope'.ucwords($scopeOpt))) $this->queryBuilder->$scopeOpt();
		}
	}
	...
	private function sortByColumn()
	{
		...
		if (!isset($this->relations[$field])) $this->queryBuilder->orderBy($field, $this->options['order'][0]['dir']); // if field exists on model
		else // if field is relation of model
		{
			...
			if ($relation instanceof \Illuminate\Database\Eloquent\Relations\BelongsTo)
			{
				...
				{
					if (is_string($otherField)) $this->queryBuilder->orderBy($otherTable.'.'.$otherField, $this->options['order'][0]['dir']);
					else
					{
			...
			else if ($relation instanceof \Illuminate\Database\Eloquent\Relations\BelongsToMany)
			{
				$this->queryBuilder
					...
					->select($table.'.*')
					->distinct();
				foreach ($this->relations[$field] as $otherField) $this->queryBuilder->orderBy($otherTable.'.'.$otherField, $this->options['order'][0]['dir']);
			}
		}
	}
	...
	{
		if (empty($this->options['search']['value'])) return false;

		if (!method_exists($this->model, 'scopeSearch')) throw new BadMethodCallException('Call to undefined method '.get_class($this->model).'::scopeSearch()');
		...
		{
			$term = trim($term);
			if (!empty($term)) $this->queryBuilder->where(function ($query) use ($term) { $query->search($term); });
		}
	...
	private function getFormatedData()
	{
		...
		return [
			...
			'data'				=> $collection->map(function($model) { return $model->getDatatablesData(); }),
		];
	}
